...
class ApiKeyManager:
    ...
    def __init__(self, runtime_key: str = None):
        """
        If runtime_key is provided, use it.
        Otherwise, fall back to environment variable or .env.
        """
        self.api_keys = {}
        if runtime_key:
            self.api_keys["OPENROUTER_API_KEY"] = runtime_key
            log.info("Using OpenRouter API key provided at runtime")
        else:
...
class ModelLoader:
    ...
    def __init__(self, runtime_key: str = None):
        # Pass runtime_key to ApiKeyManager
        self.api_key_mgr = ApiKeyManager(runtime_key=runtime_key)
...
if __name__ == "__main__":
    # Example: pass runtime key here
    runtime_key = input("Enter your OpenRouter API key: ").strip()
    loader = ModelLoader(runtime_key=runtime_key)
